...
def create_predict_data(path,img_list,out,net,dataloader,device,img_size):
    ...
    croped_outno = os.path.join(out,'predict_crop_images.no')
    ...
    gc.collect()
    import zipfile
    myZipFile = zipfile.ZipFile("data.zip", "w" )
    ...
    for i,img_name in tqdm(enumerate(img_list)):
        ...
        #if croped in covid_images:
        Image.fromarray(croped).save(os.path.join(croped_out,'croped_'+img_name))
        myZipFile.write('predict_crop_images/croped_'+img_name)
        #if croped in no_covid_images:
        #Image.fromarray(croped).save(os.path.join(croped_outno,'no_croped_'+img_name))
            #myZipFile.write("no_croped_"+img_name, "predict_crop_images_no\\no_croped_"+img_name, zipfile.ZIP_DEFLATED )
    #my_zip = zipfile.ZipFile('data.zip','w')
    #my_zip.write('predict_crop_images')
    #my_zip.write(croped_outno)
    myZipFile.close()
